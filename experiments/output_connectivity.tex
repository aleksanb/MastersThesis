...
A regression model trained on a certain reservoir might not work on one with slightly altered topology.
...
It was not investigated whether these hotswappable reservoirs were structurally similar.
If they are, one might not have to retrain the model every time the substrate evolves,
as small changes to the topology might jump to a similarly well-performing RBN.
